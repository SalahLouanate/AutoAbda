...
const INITIAL_PONTS = [
  { id: 1, nom: 'Pont 1', statut: 'Opérationnel' },
  { id: 2, nom: 'Pont 2', statut: 'Opérationnel' },
...
export default function GestionRessourcesView() {
  const [techniciens, setTechniciens] = useState(INITIAL_TECHNICIENS)
  const [ponts,       setPonts]       = useState(INITIAL_PONTS)
  ...
  const toggleActif = (id) =>
    setTechniciens(prev => prev.map(t => t.id === id ? { ...t, actif: !t.actif } : t))

  // ── Toggle statut pont ───────────────────────────────────────────────────
  const togglePont = (id) =>
    setPonts(prev => prev.map(p =>
      p.id === id
        ? { ...p, statut: p.statut === 'Opérationnel' ? 'En maintenance' : 'Opérationnel' }
        : p
    ))
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{tech.pontDefaut}</p>
                  </div>
                </div>
                  ...
                  {STATUTS_JOUR.map(statut => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all duration-150 active:scale-95 ${style[statut]}`}
                      >
                        {statut}
                      </button>
                    )
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            {ponts.map(pont => {
              ...
              return (
                <button
                  ...
                  className={`group flex flex-col items-center gap-3 p-4 rounded-xl border-2 transition-all duration-200 active:scale-95 text-center
                    ${isOk
          ...
          <div className="mt-4 flex items-center gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
            ...
            <p className="text-xs text-slate-500">
              <span className="font-bold text-slate-700">{ponts.filter(p => p.statut === 'Opérationnel').length}</span> ponts opérationnels ·{' '}
              <span className="font-bold text-slate-700">{nbMaintenance}</span> en maintenance
              {nbMaintenance > 0 && (
                <span className="text-amber-600 font-semibold">
                  {' '}— {ponts.filter(p => p.statut === 'En maintenance').map(p => p.nom).join(', ')} hors service
                </span>
              )}
            </p>
          </div>
